...
class Event():
    ...
    def process(self, img: np.ndarray, prompt: str, neg_prompt: str, sampleStep: int, guidance_scale: int):
        ...
        with torch.no_grad():
            ...
            result = self.img_ext.sample(extend_latent, masked_latent, mask, chunkList, prompt_embeds,
                                         guidance_scale=guidance_scale)
            out_img = self.img_ext.getImg(result)[0]

        out_img = cv2.resize(out_img, (newimg.shape[1], newimg.shape[0]))
        ...
        with gr.Row():
            with gr.Column():
                ...
                img_window = gr.AnnotatedImage(
                    ...
                    height=300)
                image.upload(event.image_change, inputs=[image], outputs=[img_window])
                image.clear(event.apply_to, inputs=[image])
            pre_image = gr.Image(height=600, interactive=False)

        with gr.Row():
            with gr.Column():
                prompt = gr.Textbox(interactive=True, label="prompt")
                neg_prompt = gr.Textbox(interactive=True, value="Blurred, incomplete, distorted, misplaced",
            ...
            with gr.Column():
                with gr.Row():
                    ...
                    left.click(event.left_ext, inputs=[image], outputs=[img_window])
                    right.click(event.right_ext, inputs=[image], outputs=[img_window])
                    up.click(event.up_ext, inputs=[image], outputs=[img_window])
                    down.click(event.down_ext, inputs=[image], outputs=[img_window])
                grid_size = gr.Number(128, label="grid size")
                grid_size.change(event.set_grid_size, inputs=[grid_size])
        with gr.Row():
            with gr.Row():
